...
Here we want to investigate the effect on PFI and LOCO Feature Importance of the correlated features with different kind of dependencies on the target value y. (only the szenarios which show interesting results will be shown later)
...
y1 <- data$V1 + data$V2 + data$V3 + data$V4 + rnorm(n = 1, mean = 0, sd = 1) #correlated

y10 <- data1$V1 + data1$V2 + data1$V3 + data1$V4 + rnorm(n = 1, mean = 0, sd = 1) #uncorrelated
...
```{r LOCO0, message = FALSE, fig.height=6, fig.width=12, fig.cap="LOCO Feature Importance with uncorrelated features (r = 0) on random forest model"}
library(mlr)
library(iml)
library(ggplot2)
library(randomForest)
...
```{r szenariononlm, message = FALSE, fig.height=6, fig.width=12, fig.cap=" Nonlinear Dependence"}

#y2 <- sin(data$X1) + data$X2 + data$X3 + data$X4 + 
      # rnorm(n = 1, mean = 0, sd = 1) #correlated

#y20 <- sin(data1$X1) + data1$X2 + data1$X3 + data1$X4 + 
      # rnorm(n = 1, mean = 0, sd = 1) #uncorrelated

### Simulation for y2 (nonlinear dependence) and correlated features
data <- as.data.frame(data)
df <- data
df['random'] <- runif(nrow(df))

### Simulation for y10 (nonlinear dependence) and uncorrelated features
data1 <- as.data.frame(data1)
df1 <- data1
df1['random'] <- runif(nrow(df1))
...
```{r szenariouninf, message = FALSE, fig.height=6, fig.width=12, fig.cap="Szenario Uniformative Feature"}

y3 <- data$V1 + data$V2 + data$V3 + data$V4 + rnorm(n = 1, mean = 0, sd = 1) #correlated

y30 <- data1$V1 + data1$V2 + data1$V3 + data1$V4 + rnorm(n = 1, mean = 0, sd = 1) #uncorrelated) 

```

4) Simulation with interaction effect

5) Simulation with high amount of correlated data
