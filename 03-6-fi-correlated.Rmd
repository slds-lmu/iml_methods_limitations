...
Two main problems can be derived from the following examples. By adding a correlated feature to the data set can lead to a decrease in Feature Importance is the first and most crucial problem. Imagine you want to predict the risk of a heart attack by looking at the weight of a person had yesterday and other uncorrelated features. For instances, you choose a random forest model. It is well know that overweight can be a deciding influence factor for heart attacks and your choosen model as well indicating that weight is the most important feature. What happens if you now also add the weight of the person of today which is highly correlated to the weight of a person yesterday. A big advantage of a random forest model is the application and predicitve accuarcy of high dimensional data sets. Even in the case of correlated features or interaction effects. So adding a new component should be no problem. However, there can be some effects on the Feature Importance that can make an interpreation more difficult. Since, now the importance kind of splits between both features. During the training of the random forest, some of the decision trees will choose the weight of today, the the weight of yesterday, both or another none of these two as a split criteria.   [...]
...
```{r PFIAlgo, message = FALSE, fig.height=6, fig.width=12, fig.cap= "PFI Algo"}
...
PFI <- function(data,learner,target,model){
  ...
  resultPFI <- data.frame(matrix(nrow=5,ncol=1))
    ...
    mod <- Predictor$new(model,
                          data=d[c(unlist(getResamplingIndices(res)$test.inds[i], use.names=FALSE)),1:5], 
                          y = d[c(unlist(getResamplingIndices(res)$test.inds[i], use.names=FALSE)),6])
...
```{r LOCOAlgo, message = FALSE, fig.height=6, fig.width=12, fig.cap= "LOCO Algo"}
library(mlr)
library(iml)
library(ggplot2)
library(randomForest)
...
```{r LOCO08, message = FALSE, fig.height=6, fig.width=12, echo = FALSE, eval=FALSE, fig.cap="LOCO Feature Importance with different kind of correlation intensity on a random forest model"}
library(mlr)
library(iml)
library(ggplot2)
library(randomForest)
...
```{r PFIreal1, message = FALSE, fig.height=6, fig.width=12, fig.cap= "Comparison"}
library(ggpubr)
library(corrplot)
library(randomForest)
library(mlr)
...
```{r PFIreal2, message = FALSE, fig.height=6, fig.width=12, fig.cap= "Comparison"}
library(ggpubr)
library(corrplot)
...
```{r arrange06, message = FALSE, fig.height=6, fig.width=12, fig.cap= "Comparison"}
library(ggpubr)
...
```{r LOCOreal1, message = FALSE, fig.height=6, fig.width=12, fig.cap= "Comparison"}
...
```{r LOCOreal2, message = FALSE, fig.height=6, fig.width=12, fig.cap= "Comparison"}
...
```{r arrange07, message = FALSE, fig.height=6, fig.width=12, fig.cap= "Comparison"}
library(ggpubr)
