...
```{r, fig1, eval = TRUE, echo = FALSE, fig.align = 'center', out.width = '99%', fig.cap = "In(appropriate) kernel widths for kernel density estimations. The left panel illustrates an appropriate kernel width. The right one features an inappropriate one."}
...
These weights are chosen w.r.t. proximity to the observation to be explained.
...
This could be obtained in many more ways than weighting observations as done in LIME where global sampling is applied.
After sampling, the samples are weighted w.r.t. their proximity to the observation to be explained.
One possible alternative might be to combine steps 2 (sampling) and 4 (weighting) of the LIME algorithm ^[refer to the previous chapter] to a local sampling.
...
For scaled numerical features (standard deviation is one) this metric is equivalent to the euclidean distance if categorical features are one-hot-encoded.
It is important to note that despite these measures it may be challenging to properly determine distances for mixed data. 
@ribeiro2016should recommend using cosine distance for text and Euclidean distance for images.
...
As the choice of the distance measure seems least arbitrary and the choice of the kernel function is not expected to have the most crucial impact on the neighbourhood definition, we focus on the __kernel width__ in our experimental study.
...
```{r, fig2, eval = TRUE, echo = FALSE, fig.align = 'center', out.width = '99%', fig.cap = "Simulated data: The non-linear relationship between the feature and the target."}
...
```{r, fig3, eval = TRUE, echo = FALSE, fig.align = 'center', out.width = '99%', fig.cap = "Simulated data: Random forest predictions for non-linear univariate relationship. The solid line represents the true predictive surface."}
...
For example, we could describe the data locally well by piece-wise linear models.
This is depicted in figure \@ref(fig:fig4).

```{r, fig4, eval = TRUE, echo = FALSE, fig.align = 'center', out.width = '99%', fig.cap = "Simulated data: Non-linear univariate relationship explained by a piece-wise linear model."}
...
```{r, fig5, eval = TRUE, echo = FALSE, fig.align = 'center', out.width = '99%', fig.cap = "Simulated data: Possible local (LIME) models for the non-linear univariate relationship."}
...
In figure \@ref(fig:fig6) (upper panel) we see these averaged models for different kernel sizes.
...
```{r, fig6, eval = TRUE, echo = FALSE, fig.align = 'center', out.width = '99%', fig.cap = "Simulated data: Local (LIME) models for non-linear univariate relationship with different kernel sizes for different observations."}
...
The only generic goodness-of-fit criterion of LIME, model fidelity, is not necessatily representative.
...
_Good_ machine learning models should be able to approximate the hyperplane.
...
Here, we assume that the kernel size may be infinitely large as the global model should equal good local models.
...
Using cross-validation, we in fact confirm that the model has a high predictive capacity.
...
```{r, fig10, eval = TRUE, echo = FALSE, fig.align = 'center', out.width = '99%', fig.cap = "Simulated data: Each panel represents a single (representative) observation. For each observation we analyse the LIME coefficients for different kernel widhts. The underlying ground truth model is a linear model where x1 only has a local coefficient. Each feature is depicted in a different colour. The solid vertical lines represent the true coefficient of the LIME explanation."}
...
In our scenario, good kernel widths are between $0.1$ and $0.75$ (while the upper bound depends on the observation).
As before, we observe that a too-small kernel width produces non-meaningful coefficients.
On the other hand for large kernel widhts, the true coefficient is not approximated, but rather the global (average) linear coefficient:
...
```{r, fig11,  eval = TRUE, echo = FALSE, fig.align = 'center', out.width = '99%', fig.cap = "Simulated data: For one observation we display the local coefficient and confidence intervals for different kernel widths. The underlying ground truth model is a linear model where x1 only has a local coefficient. Hence, we only investigate x1."}
...
The resulting trade-off suggests choosing the minimal kernel size with stable coefficients.
...
Figures \@ref(fig:fig12) and \@ref(fig:fig13) look very similar to the corresponding figures of the previous section.
...
```{r, fig12, eval = TRUE, echo = FALSE, fig.align = 'center', out.width = '99%', fig.cap = "Simulated data: Local coefficients for different kernel widths explaining non-linear relationship (for x2)."}
knitr::include_graphics("images/04-09-12.png")
```

```{r, fig13, eval = TRUE, echo = FALSE, fig.align = 'center', out.width = '99%', fig.cap = "Simulated data: Local coefficients and confidence intervals for different kernel widths explaining non-linear relationship (for x2)."}
...
Thus, LIME should always apply to scale.

3. When working with real data sets with many features, we typically want a sparse explanation.
To achieve this, we should let LIME do the feature selection.
...
Our motivation to use stability paths is that they are easier to interpret compared to coefficients paths (or similar) in our setting.
...
Figure \@ref(fig:fig131) displays ideal stability paths with the three distinct areas observed earlier:
...
```{r fig131, eval = TRUE, echo = FALSE, fig.align = 'center', out.width = '99%', fig.cap = "Real data: Example for ideal stability paths with three distinct areas."}
...
```{r fig14, eval = TRUE, echo = FALSE, fig.align = 'center', out.width = '99%', fig.cap = "Real data: Stability paths for different observations from the bycicle data set explaining a random forest."}
...
Then, either the windspeed (e.g. only observation 1) or the season (the remainder) is selected as global feature.
We believe that this is due to the fact that both covariates are similarly strong on the global scope.
Globally evaluating the feature importance of the random forest suggests that in fact temperature, season, humidity and windspeed are the most relevant features.
...
For instance, consider observation 1 and 3.
Here, the local models seem quite clear.
For observation 1, between kernel widths of $0.5$ and $1$ it seems as if the temperature, the windspeed and the weather situation are most influential.
For observation 3, the selected local features are temperature, windspeed and season.
For other observations, such as observations 4 and 8, we may argue that there are local and stable local explanations, too.
...
As distances in higher dimensional Euclidean space are increasingly less meaningful, the defintion of locality is very poor with an increasing number of features.

We observe both effects being described in the literature for our real data example: 
...
At the same time, we are not able to identify local and stable explanations in this controlled environment.
...
The specification of a proper kernel width is crucial to achieve this.
...
Using the lime package we heavily deviate from the default package options.
...
In figure \@ref(fig:fig15) we analyse this statement by comparing the Gower distance's local coefficient to the true coefficient and the local estimates of the non-linear data simulation from section \@ref(id413).

```{r fig15, eval = TRUE, echo = FALSE, fig.align = 'center', out.width = '99%', fig.cap = "Simulated data: Gower distance vs. Euclidean distance (non-linear realtionship). The blue line is the true coefficient. The interpolated curve represents the LIME coefficient estimates for different kernel widths when using Euclidean distance. The green line represents to estimate resulting from LIME when using Gower distance."}
knitr::include_graphics("images/04-09-16.png")
```

We see that our argument is valid.
Gower distance is not able to recover the true coefficients and acts like a global surrogate.
...
Usually, LASSO is the preferred option for variable selection as it is less seed dependent as, let's say, step-wise forward selection.
