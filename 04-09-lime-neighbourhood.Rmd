...
However, models like a random forest, which are not good at extrapolating, may produce implausible predictions for many permutations possibly causing less stable explanations.
For our simulated data, all of our results are _qualitatively_ identical between MARS and random forest, though.
...
The model performs (on average) reasonably well with a mean absolute error 952 on the held-out data while the mean of the bicycle rental count is 4284. (MAKE CV!!)
...
Below in figure \@ref(fig:fig14), we display the stability paths for different observations for the random forest.
