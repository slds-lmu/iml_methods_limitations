...
In theory, MARS can reproduce the data generating process perfectly and hence our first choice.
...
We investigate _representative_ observations, i.e. one belonging to each _bin_ of the predictive surface to check if LIME recovers all local coefficients.
...
This can also be interpreted as a global surrogate.
...
The stability paths suggest that temperature, weather situation and holiday are the local features while temperature, humidity and wind speed are deemed as the global ones.
...
We were unable to detect explanations that were both, stable and local, for our real data application -- at least with certainty.
...
This argument makes binning -- provided that there is no additional benefit except interpretability (cross-reference next chapter) -- obsolete.
