...
For our simulated data, all of our results are qualitatively identical between MARS and random forest.
...
To illustrate this, we investigate _representative_ observations, i.e. one belonging to each _bin_ of the predictive surface in figure \@ref(fig:fig9).
...
Using this data we aim to use both, a MARS model and a random forest to predict the number of daily bicycle hires. 
We tune them using cross-validation.
For both, we use LIME to explain the black box.
Both models perform (on average) reasonably well with a mean absolute error of 1000 (MARS) and 952 (random forest) on the held-out data while the mean of the bicycle rental count is 4284. (MAKE CV!!)
...
Below in figure \@ref(fig:fig14) and \@ref(fig:fig15), we display the stability paths for different observations for random forest and MARS.
...
(CHECK)
We observe both effects described in the literature: instability (@alvarez2018robustness) for small kernel widths and global surrogates (@laugel2018defining) for larger ones which we already discovered for simulated data.
For real data, however, it is hard to locate the area we identified for simulated data where we find a stable __and__ local model.
For example, consider the random forest.
We may conclude that in fact for observations 1 and 3 a stable and local explanation would involve the season, the temperature and the weather situation.
Both observations...
However, for observation 3, a possible (less) stable explanation would also involve the season, the temperature and the indicator for a working day.
... more and earth ...
