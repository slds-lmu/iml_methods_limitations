...
We set $\beta_1 = 4$, $\beta_2 = -3$, $\beta_1 = 5$ and $c_1 = 6$ and $c_2 = 4$.
...
3. $x_1 < 6$ and $x_2 > 4$.
4. $x_1 > 6$ and $x_2 > 4$.
We think these observations are best explained in areas with reasonable margin to $c_1$ and $c_2$.

Below we depict the coefficient paths for four representative observations.
We can clearly see that in this case we cannot simply set an arbitrary kernel width.
The true local coefficients are only approximated well within a narrow interval of the kernel width.
...
We observe that there is a trade-off between stable coefficients and locality.
