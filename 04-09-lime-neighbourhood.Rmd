...
If LIME worked as it is supposed to, we would expect two things:
First, the alternation of the kernel width should in fact affect the explanations. 
However, in a smooth way. 
This means that a small change in the kernel width should not affect the explanation much.
Second, for a given kernel width we expect stable explanations for identical instances. 
This means if we repeatedly computed LIME explanations we would expect that they are similar.

To investigate these expectations we simulate data where we know the true decision boundaries that LIME is supposed to explain.
We will work with linear, non-linear and interacted relationships in the simulated data.
The underlying problem is a regression problem which we aim to solve with a random forest.
We fit the black box model leave it fixed.
Then, for all simulated data sets we obtain LIME explanations for randomly selected instances.
We examine the development of the coefficient values corresponding to the respective kernel width.
We do so by investigating their "stability paths".
As 
