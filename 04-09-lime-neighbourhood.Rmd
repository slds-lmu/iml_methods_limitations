...
We, somewhat surprisingly, see that they look very similar for all observations for each model.
It seems as if both models have a slightly different order of important features, though.
We observe both effects described in literature: instability (@) for small kernel widths and global surrogates (@) for larger ones.
For real data, however, we cannot observe the area we identified for simulated data where we find a stable & local model.
(only some tendencies...)




Interestingly, our results remain qualitatively identical when we use the example from the R vignette instead (only changing to the euclidean distance instead).
However, our result agrees with the findings of @laugel2018defining who find LIME to be biased towards global features.
