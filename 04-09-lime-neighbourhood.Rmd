...
Hence, we will limit ourselves on the analysis of tabular data for the remainder of this section.

As described in the previous chapter, LIME aims to create local surrogate models that operate in the proximity or _neighbourhood_ of the instance to be explained.
Typically, this neighbourhood is achieved by a weighted model.
The weights come from a kernelised distance.

Proper parametrisation of the kernel is an obvious issue.
...
@huang1998kproto recommends to set $\lambda$ to equal to the average standard deviation of the numerical features.
...
However, it is important to note that despite these mesasures it may be challenging to properly determine distances for mixed data properly. 
@ribeiro2016should recommend using cosine distance for text, euclidean distance for images.
...
Binning is useful to achieve improved interpretability while it causes problem when sampling at the same time.
...
In order to improve interpretability when working with numerical features we recommend to multipy the resulting local coefficients with the feature value to restore this kind of interpretabilty.
...
We use a linear regression (the true model) as black box model.
...
```{r fig7, eval = TRUE, echo = FALSE, fig.align = 'center', out.width = '99%', fig.cap = "Simulated data: Model evaluation for a linear model."}
...
Note that a random forest would have the same property.
However, models like a random forest, which are not good at extrapolating, may produce implausible predictions for many permutations.
For our simulated data, all of our results are qualitatively identical between MARS and a random forest.
The random forest explanations are less stable and tend to be more biased towards a global surrogate ceteris paribus.
...
```{r, fig9, eval = TRUE, echo = FALSE, fig.align = 'center', out.width = '99%', fig.cap = "Simulated data: MARS predictions for piece-wise local associations."}
...
When explaining the black box with LIME we observe a same pattern as before in figures \@ref(fig:fig12) and \@ref(fig:fig13).
However, the interval if "good" solutions is -- naturally -- much smaller.
...
2. Computing some dissimilarities (e.g. Manhattan or euclidean) also comes with the problem that the cardinility of the features mainly steers this meausre.
...
We will work with the well-known Washington D.C. bycicle rental data set.
This dataset contains daily bycicle hire counts of a Washington D.C. based rental company. The data has been made openly available by company itself (Capital-Bikeshare). 
...
We select this data set because this it is well-known in the machine learning community and this regression problem is accessible to most people.
...
Using this data we aim to use both, a MARS model and a random forest to predict the number of daily bycicle hires. 
We tune them using cross validation.
For both, we use LIME to explain the black box.
Both models perform (on average) reasonably well with a mean absolute error of ...
...
We observe that stability paths for all observations converge to the same set of covariates if we set the kernel width large.
These are the global features.
In our example we find... to be the four most important global features.
...
Below in figure \@ref(fig:fig14) and \@ref(fig:fig15), we display the stability paths for different observations for a random forest and MARS.
...
```{r fig15, eval = TRUE, echo = FALSE, fig.align = 'center', out.width = '99%', fig.cap = "Stability paths for the bycicle data set explaining a MARS model."}
knitr::include_graphics("images/04-09-15.png")
```

We, somewhat surprisingly, see that they look very similar for all observations for both models.
MARS only seems to faster stabilise than the random forest.
It seems as if both models have a slightly different order of important features, though.
We observe both effects described in literature: instability (@alvarez2018robustness) for small kernel widths and global surrogates (@laugel2018defining) for larger ones which we already discoverd for simulated data.
For real data, however, we cannot locate the area we identified for simulated data where we find a stable __and__ local model.
Only for the random forest and observation 5 we might detect some local dependencies.
This, however, is speculative as it is a pattern we do only rarely observe.
The only stable explanations are essentially the global explanations.
It seems as if the predictive surface is too complex to facilitate local and stable surrogate models.

Our results remain qualitatively identical when we use the example from the R vignette instead (only changing to the euclidean distance instead of gower).
...
In fact, we were unable to find explanations that were both, stable and local, for our real data application.
...
We confirm this finding using the bicycle rental data set. 
Furthermore, we already see these tendencies when increasing the complexity of the simulated data.
Note that we in fact found similar behaviour for many different data sets.
For the practioner using LIME this means...

We propose that LIME should be altered in the way proposed by @laugel2018defining to LIME-K.
This means that the locality should already affect the sampling and global sampling should be prefered.
...
The problems described within this sections are mainly associated with tabular data.
...
For example, LIME's sampling for text data is very local, only creating pertubations based on the instance supplied.
