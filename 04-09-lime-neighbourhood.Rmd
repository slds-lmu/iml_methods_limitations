...
So does Gower's distance @gower1986metric or a bit more general the distance proposed by @huang1998kproto
...
@huang1998kproto recommends to set $\lamdba$ to equal to the average standard deviation of the numerical features.
...
When explaining the black box with LIME we observe a same pattern as before.
...
Sparse means that we limit our explainer to only the four most influential local features.
...
Below, we display the stability paths for different observations for MARS and random forest.
...
Interestingly, our results remain qualitatively identical when we use the example from the R vignette instead (only changing to the euclidean distance instead of gower).
