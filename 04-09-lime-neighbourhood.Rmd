...
This particular instance can be linearly described approximately by linear regression with intercept $60$ and slope $-4.5$.
...
As a next step, we review explanations resulting from altering the kernel size in figure \@ref(fig:fig6) more systematically. 
...
```{r fig131, eval = TRUE, echo = FALSE, fig.align = 'center', out.width = '99%', fig.cap = "Real data: Example for ideal stability paths with three distinct areas. The x-axis displays different kernel widths. The y-axis indicates the respective inclusion probability of each variable. The variables are grouped by colour."}
