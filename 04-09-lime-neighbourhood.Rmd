...
Below we indicate the test set performance of the model in figure X.
...
We observe that too small kernel widths are not able to reproduce the global predictive surface.
...
This means that for predicitive surfaces that correspond to linear relationships the kernel width is a non-critical parameter.
