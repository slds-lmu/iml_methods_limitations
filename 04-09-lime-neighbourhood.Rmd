...
This time, we fit a MARS model @friedmann which can deal with this property of local features.
...
Below, we display the stability paths for different observations.
We, somewhat surprisingly, see that they look very similar for all observations.
We see that most observations deem the same set of covariates to be influential already for the minimal kernel width granting stable results.
For some, however, we see that for small and stable kernel widths different models are fit than the global one.
This..
