...
data <- mtcars %>% setDT
features <- colnames(data)[c(2:11)]
feature_names <- c("Number of cylinders", "Displacement", "Horsepower", "Rear axle ratio", "Weight", "1/4 mile time","V/S", "Transmission", "Number of gears", "Number of carburetors")

## Wie viele Wiederholungen fÃÂ¼r Konfidenzintervall?
n.conf <- 100

set.seed(04022019)
mx <- matrix(data = 0, nrow = n.conf, ncol = length(features))
colnames(mx) <- feature_names
## 1.5 min
for(i in 1:n.conf){
  ...
  ## Wenn interne Variable Importance genutzt werden soll:
...
conf.intervalls <- matrix(nrow = 3, ncol = length(features))
colnames(conf.intervalls) <- feature_names
rownames(conf.intervalls) <- c("Minimum", "Mean", "Maximum")
for(i in 1:length(feature_names)){
  
  conf.intervalls[1,i] <- quantile(mx[,i], 0.05)
  conf.intervalls[2,i] <- mean(mx[,i])
  conf.intervalls[3,i] <- quantile(mx[,i], 0.95)
  
}


## Mit Errorbar
conf.int <- as.data.table(conf.intervalls %>% t())
conf.int$Variable <- feature_names
conf.int[, Variable := ordered(Variable, levels = feature_names[order(conf.int$Mean)])]
p1 <- ggplot(data = conf.int, aes(x=Variable, ymin=Minimum, ymax=Maximum)) + 
  geom_errorbar(width=0.3, size=1, color="darkblue") + 
  geom_point(mapping=aes(x=Variable, y=Mean), size=3, shape=21, fill="white") +
  coord_flip() +
  theme(plot.subtitle = element_text(size = 10)) +
  labs(y = "Mean Feature Importance", x= "") + 
...
$L(y,x)- L(y,x_{-1}) =  \text{Feature Importance of }x_{1}$
...
<center>
$L(y,x) - L(y,x_{-p}) = \text{Feature Importance of } x_{p}$
</center>
...
<!--chapter:end:03-feature-importance.Rmd-->
