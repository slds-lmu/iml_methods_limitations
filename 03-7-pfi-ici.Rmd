...
In the subchapter "Detect Interations", it will be focussed on to what extent the PI and ICI plots can uncover interaction effects between features. In order to give this a new angle of perspective, a new method, called "derivative-ICI" will be introduced. 
...
The whole chapter is then closed by a brief summary and discussion of the methods. This will include a final evaluation of the PI and ICI plots and thereby answer the question whether the methods are useful or not. 
...
In order to supplement the results, yielded from the PI and ICI plots, a method will be proposed that was introduced by (...) with the purpose to detect interaction effects in the context of analyzing Feature Effects. 
...
Here, it is still not clarified which learner is the most suited. Ideally, one would like yield sparse results where noise is not fitted. This would prevent that other features, which are actually independent, are included in the model. This property would hold for any regularized regression model. Yet, it would be additionally convenient if the learner would additionally output the threshold were the interaction takes place. 

Both desired properties hold best when inducing a decision-tree with tree-depth of 1. This configuration allows to ignore noise and the returned split point indicates the threshold for which the conditional Feature Importance should be calculated. 
...
Further, it was stated that explaining interaction is as important as detecting interaction effects. Even if the ICI plots were able to detect interaction effects, it was not possible to really explain them. Briefly, between which features and where did the interaction take place. 
...
Even though, the PI and ICI plots themselves are very limited in its explanatory power, it was still possible with the underlying data to create a cookbook which enabled a full picture. The d-ICI plots represents a robust method for detecting interactions, even in a "messy" non-linear relationship. It turned out that calculating the approximated Feature Importance was easy to implement and therefore posed no major challenge. Further, as the Partial Importance was already calculated it was easy to implement a method which explains between which features interaction takes place. With this information, one can calculated the conditional Feature Importance and therefore, finally visualize actual effect of the interaction on the Feature Importance. 
