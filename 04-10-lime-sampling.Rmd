...
filename = paste0("images/sd_pc.png")
png(filename, width = 700, height = 500)
plotc
dev.off()
...
plot_data = data.frame(
  sd = apply(results_sd[-1], MARGIN = 1, function(row) mean(row[!is.nan(row) & !is.na(row)])),
...
filename = paste0("images/sd_nfeatc.png")
png(filename, width = 700, height = 500)
plotc
dev.off()
...
#summary(results_sd)
# bikes
# holy shit (change this if no dummy encoding is used)
...
plot = ggplot(plot_data, aes(y = sd, x = smoothness, color = type)) +
  geom_line() +
  geom_line(data = errors_data, aes(y = mse/max(mse) * 0.02, x = smoothness, color = NULL)) +
  theme_minimal() +
  theme(
    text = element_text(size = 15),
    axis.title.x = element_text(vjust = -4),
...
filename = paste0("images/sd_overfitting.png")
png(filename, width = 700, height = 500)
plot
dev.off()
