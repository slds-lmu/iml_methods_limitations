...
In this chapter we will discuss the fundamentals of LIME from a slightly different angle to receive further understanding of what enables sampling and to have a look how basic results look like.


### Formula

If we do some small changes of notations compared to the introductional chapter, the task of calculating the LIME explainer can be seen as
...
This may sound absolutely great at first, but we still need values of our feature space for evaluation.
...
Our original dataset is a finite sample of an infinite space in the numerical case, or of a finite space exponentially growing with its dimension in the categorical case.
...
Originally, sampling in LIME was meant as a perturbation of the original data, to stay as closely as possible to the real data distribution. [cite]
...
But in theory issues could occure, if a lot of unrealistic evaluations lied close to our point for explanation and influenced greatly the fit of the surrogate model.
...
Since the original data is thrown away after sampling, no information is left over about this category for the fitting process.
...
The small benefit here is being allowed to fine tune the amount of bins, leading to a rougher or more detailed sample representation of the original feature.
...
Additionally, it is not possible to change options for each feature, so by chosing this distribution, all your features will be handled as normally distributed with their own individual mean and variance.
