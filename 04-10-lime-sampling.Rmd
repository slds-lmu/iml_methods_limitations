...
<!-- Way too many plots!, try to reduce it to the essence or at least work with panels -->
...
png(filename, width = 700L, height = 500)
...
<!-- Really so bad? Well if the obs is not normal why expect the model to behave nicely? -->
...
<!-- AGAIN: Way too many plots. Better show only some and rather focus on it.
Then just describe the other results in your text.
You cannot show the reader everything you did.
You already overloaded me being familiar with the topic.
...
<!-- You gave me too much input. I cannot judge what is most important / interesting -->
...
results  = readRDS("feature_growth_boston_repeats100")
results$type = "kernel_est"
...
#  use_density = TRUE
#)
#saveRDS(resultsb, "feature_growth_boston_repeats100_bins")
resultsb = readRDS("feature_growth_boston_repeats100_bins")
resultsb$type = "bins"
...
#  use_density = TRUE
#)
#saveRDS(resultsn, "feature_growth_boston_repeats100_nd")
resultsn = readRDS("feature_growth_boston_repeats100_nd")
resultsn$type = "normal_dis"
...
#  type = "regr", dim_increment = 1L, repeats = 100,
#  bin_continuous = TRUE,
#  use_density = TRUE
#)
#saveRDS(resultsc, "feature_growth_bikes_repeats100")
resultsc = readRDS("feature_growth_bikes_repeats100")
...
filename = paste0("images/sd_p_presi.png")
png(filename, width = 700L, height = 500)
plot
dev.off()
...
filename = paste0("images/sd_pc_presi.png")
png(filename, width = 700L, height = 500)
plotc
dev.off()
...
filename = paste0("images/sd_nfeat_presi.png")
png(filename, width = 700L, height = 500)
plot
dev.off()
...
filename = paste0("images/sd_nfeatc_presi.png")
png(filename, width = 700L, height = 500)
plotc
dev.off()
...
#)
#saveRDS(results, "permutation_growth_boston_repeats100_perm1kx2kx4kx8kx16kx32k")
results = readRDS("permutation_growth_boston_repeats100_perm1kx2kx4kx8kx16kx32k")
results$type = "kernel_est"
...
#  use_density = TRUE
#)
#saveRDS(resultsb, "permutation_growth_boston_repeats100_perm1kx2kx4kx8kx16kx32k_bins")
resultsb = readRDS("permutation_growth_boston_repeats100_perm1kx2kx4kx8kx16kx32k_bins")
resultsb$type = "bins"
...
#  use_density = TRUE
#)
#saveRDS(resultsn, "permutation_growth_boston_repeats100_perm1kx2kx4kx8kx16kx32k_nd")
resultsn = readRDS("permutation_growth_boston_repeats100_perm1kx2kx4kx8kx16kx32k_nd")
resultsn$type = "normal_dis"
...
#  use_density = TRUE
#)
#saveRDS(resultsc, "permutation_growth_bikes_repeats100_perm1kx2kx4kx8kx16kx32k")
resultsc = readRDS("permutation_growth_bikes_repeats100_perm1kx2kx4kx8kx16kx32k")
resultsc$type = "categorical"
...
filename = paste0("images/sd_npermutations_presi.png")
png(filename, width = 700L, height = 500)
plot
dev.off()
...
# this may take a while
#results = complexity_growth(boston, "medv", pts_to_predict, type = "regr", repeats = 100L)
#saveRDS(results, "complexity_growth_boston_xgboost10")
results = readRDS("complexity_growth_boston_xgboost10")
results$type = "kernel"

# this may take a while
#resultsb = complexity_growth(boston, "medv", pts_to_predict, type = "regr", repeats = 100L, bin_continuous = TRUE, use_density = TRUE)
#saveRDS(resultsb, "complexity_growth_boston_xgboost10_bins")
resultsb = readRDS("complexity_growth_boston_xgboost10_bins")
resultsb$type = "bins"

# this may take a while
#resultsn = complexity_growth(boston, "medv", pts_to_predict, type = "regr", repeats = 100L, use_density = TRUE)
#saveRDS(resultsn, "complexity_growth_boston_xgboost10_nd")
resultsn = readRDS("complexity_growth_boston_xgboost10_nd")
resultsn$type = "normal_dis"
...
#resultsc = complexity_growth(bikes, "cnt", pts_to_predict, type = "regr", repeats = 100L, bin_continuous = TRUE, use_density = TRUE)
#saveRDS(resultsc, "complexity_growth_bikes_xgboost10")
resultsc = readRDS("complexity_growth_bikes_xgboost10")
resultsc$type = "categorical"
...
#summary(results_sd)
# bikes
#feat = names(resultsc)[50:97]
#resultsc_sd = eval(parse(text = paste0(
#  "as.data.frame(dplyr::summarize(
#    resultsc_gr,",
#    paste0("sd_", feat, " = sd(as.numeric(as.character(", feat, ")), na.rm = TRUE)", collapse = ", "),
#  "))"
#)))
...
filename = paste0("images/sd_overfitting_presi.png")
png(filename, width = 700L, height = 500)
plot
dev.off()
...
```{r, eval = TRUE, echo = FALSE, fig.align = 'center', fig.cap = "Average standard deviation"}
...
filename = paste0("images/sd_smoothness_presi.png")
png(filename, width = 700L, height = 500)
plot
dev.off()
...
knitr::include_graphics("images/sd_overfitting.png")
```
...
png(filename, width = 700L, height = 500)
presi_plot(means, lower = ci_lower, ci_upper, color1 = "#62b1e7", color2 = "#6274e7", angle = 45L, hjust = 1L, ylim = c(-0.6, 0.7))
dev.off()
