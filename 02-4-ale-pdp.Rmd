...
In difference to the ALE for one feature of interest, the 2D ALE (\@ref(eq:ale2DEst) is a two dimensional stepfunction, so there is no smoothing or something similar to make it a continuous function. 
...
```
(ref:runtime1DnumericColAndSizeCap) Runtime comparison ALE vs. PDP for one numeric feature. Differences for number of features and grid size.
