...
\@ref(eq:ale2DTheoretical1stLvl)

Following is the 2nd order effect with no correction for main effects of $x_j$ and $x_l$. So this is not yet the pure 2nd order effect the 2D ALE is aiming at.
...
\end{equation}$$
Now from the uncorrected 2nd order effect the two main effects of both features on the uncorrected 2D ALE are substracted. In this way the main effects of $x_j$ and $x_l$ on the final ALE $ALE_{\hat{f},~j,~l}(x_j, x_l)$ are both zero (TODO: apley). But be carefull, this is not centering by a constant as in the one dimensional ALE. This is a correction for the also accumulated main effects which of course vary in the directions of the features.
...
The estimation equvalent to Formula TODO is:
...
Now the correction for the main effects is estimated (compare formula TODO):
...
$$ \widetilde{ALE}_{\hat{f},~1,~2}(x_1, x_2) = \widetilde{\widetilde{ALE}}_{\hat{f},~1,~2}(x_1, x_2) ~ -  \int_{z_{0,~1}}^{x_1}  E[\frac{\delta\widetilde{\widetilde{ALE}}_{\hat{f},~1,~2}(X_1, X_2)}{\delta X_1}\mid X_1 = z_1]~dz_1 ~ $$ 
