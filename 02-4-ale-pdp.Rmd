...
The correction for the main effects extracts the effect of the single features on the ALE so far and substracts it from the raw formula (see \@ref(eq:ale2DEstCorrection)). To stick with this example the correction for the main effect of feature $x_j$ for values in $N_j(4)$ takes into account all cells in the first 4 columns and aggregates the first order effect. In cell $N_{\{j,~l\}}(4,3)$ this would look like this:
