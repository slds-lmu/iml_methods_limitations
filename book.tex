...
reasonable way. This will be explained in section 3.4..
...
by \(z_{k, ~j}\) as well as the lower boundary by \(z_{k-1, ~j}\).
...
step. Therefore the folowing formula proposed by \citep{Apley2016},
\citep{(bib[["Apley2016"]])} with slightly changed notation will be
useful. The definitions of its components are as above. Additionally
\(k_j(x)\) is defined as the number of the interval that contains \(x\),
i.e. \(x \in ~]z_{k_j(x)-1,~j},~z_{k_j(x),~j}]\).
...
``new'' datapoints. Figure \ref{fig:dataALE} gives a good intuition for
this procedure.

\begin{figure}
\includegraphics[width=13.75in]{images/ale_estimation_intuition} \caption{The data points within the 4-th interval are shifted
...
have a look on the intuition behind the uncentered ALE. If the value of
...
of both features and \(x_1\) equals 2, one would expect the prediction
to be 2 higher than the average prediction for an average data point of
