...
We only want to have a very __local__ fit of the surrogate model in the neighbourhood of the instance whose prediction is explained. 
...
For categorical features, perturbation may be realised by randomly changing the categories of a random amount of features, or even recombining all possible levels of these, covering the most important subspaces of the finite feature space as a consequence.
But this procedure is not feasible for numerical values due to an infinite space.
In this case, we may sample new values out of an approximative distribution for each feature respectively.
This way, we achieve a rather dense representation of the feature space for lower dimensions, but by ignoring the covariance structure we may run into the curve of dimensionality for higher dimsenions, requiring an exponential amount of samples.
Of course, there's a computational limit for sample amounts, which may result in a rather sparse feature representation, thus giving samples a drastical influence on the coefficient stability for unstable black box models.
(#Kommentar an Philipp, da mir das bis heute nicht klar war: "unstable models" hat mir mein chef erklärt und meint hier starke schwankungen in der prediction bei minimalen feature änderungen)
The perturbed data set, which is used to train the explainer, is much larger than the original one and supposed to better represent the (possible) feature space, giving the surrogate model more anchor points - especially in sparse areas.
