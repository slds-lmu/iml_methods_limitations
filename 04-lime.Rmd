...
However, it may be interesting to examine models locally - on the basis of
single predictions.
...
The function fit by this model operates in the complete feature space and can even yield predictions for instances not seen in the training data.
...
This is because we don't want the explainer to be ill-located in space so that it does not have anything in common with the original problem anymore.
This is why in LIME we perturb the original data.
...
LIME's perturbation of numerical data also captures the possible feature space better than the original data.
...
In fact, the way numerical features are handled is one of the main limitations of LIME
However, this limitation will not be discussed in detail in this book.
Moreover, perturbation of the data itself is very critical to LIME and may be suject to criticism.
This topic will be discussed in detail in chapter X and thus is not outlined here.

Let's come back to our formula from the beginning.
The range of the function is composed by all possible elements $g$ in our predefined set of interpretable functions $G$. 
...
Furthermore, the loss-like fidelity term $\mathcal{L}\left(f, g, \pi_x \right)$ gives a good idea how well our interpretable model fits the black box model in the local neighborhood around our target defined by the proximity measure.
The smaller this term the more we can trust our resulting model to give a good local representation. 
On the other hand, if this term is high, we should rather rethink our approach as the result will most likely not be representative for the predictive model - not even locally.

Having discussed the first two open questions, which data the explainer works with and what neighbourhood refers to, we can focus on the third question:
...
This means the result will focus on the $K$ most important parameters and fully ignore less significant ones that would give too much clutter to our interpretation.

Putting it all together we end up with the following (possible) algorithm for LIME:
...
For example, we see that the model $f$ gives no restrictions on the other choices in the equation, which gives us the opportunity to drastically change the underlying predictive model while keeping the same interpretable model with the same complexity constraints. 
...
Hence, it is likely that in many use cases LIME explanations do not fit any given context.
