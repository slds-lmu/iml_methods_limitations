...
Then, we fit a black box model that can perfectly distinguish between the two classes - indicated by the sinus shaped function.
...
Now, the data point, we want an explanation for, is colored yellow.
With respect to this point we weight our data by giving close observations higher weights.
Afterwards, we fit a classifaction model based on these weights which yields an interpretable linear decision boundary. As we can see, this is indeed locally very similar to the black box decision boundary and seems to be a reasonable result.
>>>>>>> a410609e8e8c2ff53a5a54bdd99f86e28327a9d5
