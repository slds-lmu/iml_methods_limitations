...
Models are simplifications of the reality.
...
The scientific approach suggests to simplify reality by a __model__. 
...
3. Retrieve a predictions for $Z$ using the black box model $f$.
...
(However, we may argue that our pertubation strategy is to use the original data.
...
$X$ can be an instance which is entirely new to us as long as it can be represented in the same way as the training data of the black box model.
...
Having answered the first two open question we still have the last question related to the data and the pertubation unresolved.
Besides the tabular data case, we can also interpret models trained on more complex data, like text data or image data. 
However, some data representations (e.g. word embeddings) are not human-interpretable and must be replaced by interpretable variants (e.g. one-hot-encoded word vectors) for LIME to finally yield interpretable results.
...
But how does pertubation work?
For categorical features pertubation may be realised by recombining all possible levels of the features while for numerical features we may draw new data out of normal distributions parametrised with the feature means and variances.
...
However, pertubation is associated with many practical problems when using LIME.
...
However, then we replace the random forest by a deep neural network could.
...
On the other hand, LIME being a very generic approach also means that many hyperparameters, like the neighbourhood definition or the sampling/pertubation strategy, are arbitrary.
In software implementations (R and Python) the user has no opportunity to change them manually.
The user needs to work with the default options.
...
They will investigate how these affect the results of the method and their interpretability and trustworthiness.
