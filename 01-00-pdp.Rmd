...
An exemplary plot based on the 'Titanic' data set which contains information on 2224 passengers and crew members is given in figure \@ref(fig:plot1).
...
knitr::include_graphics('images/PDP_Plot_1.pdf')
```

In classification problems with probability outputs, such as the 'Titanic' example, the partial dependence function is modeled separately for all of the K different classes. It maps the predictions for each respective class at given feature values of $x_S$.\citep{hastie2013elements} 

For categorical features, the partial dependence function and the resulting plot are produced by replacing all observed $x_S$-values with the respective category and averaging the predictions. This procedure is repeated for each of the features' categories.\citep{molnar2019} As an example, \@ref(fig:plot2) shows the partial dependence for the survival probability prediction model and the categorical feature 'passenger class'.

```{r plot2, echo=FALSE, out.width='80%', fig.cap="The classification PDP for survival probability and passenger class reveals that passengers in lower classes also had a lower probability to survive than those in a higher class.", fig.align= 'center'}
knitr::include_graphics('images/PDP_Plot_2.pdf')
```
...
```{r plot3, echo=FALSE, out.width='90%', fig.cap="Two-dimensional PDP for survival probility prediction model and numerical feature 'Age' together with the categorical feature 'Sex'. The PDP shows that while the survival probability for both genders declines as age increases, there is a difference between genders an that the decrease is much steeper for males.", fig.align= 'center'}
knitr::include_graphics('images/PDP_Plot_3.pdf')
```

```{r plot4, echo=FALSE, out.width='100%', fig.cap="Two-dimensional PDP for survival probility prediction model and numerical features 'Age' and 'Fare'. The PDP illustrates that survival probability of younger passengers is fairly uniform, while adults travelling at a lower fare also had a much lower probability to survive compared to those that paid a high fare.", fig.align= 'center'}
knitr::include_graphics('images/PDP_Plot_4.pdf')
```

Drawing a PDP with one or two feature variables allows a straight-forward interpretation of the marginal effects. This holds true as long as the features are not correlated. Should the independence assumption be violated, the partial dependence function will produce unrealistic data points. Furthermore, opposite effects of heterogeneous subgroups might remain hidden through averaging the marginal effects, which could lead to wrong conclusions.\citep{molnar2019}
...
knitr::include_graphics('images/PDP_Plot_6.pdf')
```
